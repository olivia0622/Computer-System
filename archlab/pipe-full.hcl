...
##  As what we can see IADDQ D,rB therefore it has a valC, rB，therefore it need valC in the fetch stage,but ##  do not need srcA as the input in the decode stage. In the execute stage, it will select valC as the input ##  A for ALU. In the other parts, iaddq is just like OPQ, therefore i modify as follows
####################################################################
